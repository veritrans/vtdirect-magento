...
class Veritrans_Bcava_PaymentController
  ...
  public function redirectAction() {
    ...
    Veritrans_Config::$isProduction =
        Mage::getStoreConfig('payment/bcava/environment') == 'production'
        ? true : false;

    Veritrans_Config::$serverKey =
        Mage::getStoreConfig('payment/bcava/server_key_v2');
    ...
    $payloads['payment_type']		 = 'bank_transfer';
	  $payloads['bank_transfer']		 = array(
											'bank' => "bca",
                      'va_number' => $va->getTimestamp(),
                      'inquiry' => $free_text_inquiry,
                      'payment' => $free_text_payment
										);

   
    try {
       Mage::log(print_r($payloads,TRUE),null,'bcava_request.log',true);
      $response = Veritrans_VtDirect::charge($payloads);
	  Mage::log(print_r($response,TRUE),null,'bcava_response.log',true);

    error_log($response->va_numbers[0]->va_number);
      if($response->status_code=='201') {
        /* send an order email when redirecting to payment page although payment
       has not been completed. */
		$order->setState(Mage::getStoreConfig('payment/bcava/'),true,
    ...
    Mage::getSingleton('core/session')->setBcaVa($response->va_numbers[0]->va_number);
    Mage::getSingleton('core/session')->setAmount($response->gross_amount);
    //remove item
    ...
    Mage::log($e,null,'bcava_error.log',true);      
