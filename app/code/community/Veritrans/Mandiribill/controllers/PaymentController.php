...
class Veritrans_Mandiribill_PaymentController
  ...
  // new email order
	public function send_new_order_mail($storeId, $order, $billing , $payment, $email, $name, $isGuest, $virtual='', $amount, $bill_key, $biller_code){
    ...
    else if(!empty($bill_key) && !empty($biller_code))
    {
      $url = "https://support.veritrans.co.id/hc/en-us/articles/215242768-How-to-pay-with-Mandiri-Bill-Payment-Mandiri-Virtual-Account-";
      $virtual="<h4>Please pay with Mandiri ATM IDR ".$amount." <b>your bill key:  ".$bill_key.", your biller code:".$biller_code." </b></h4><br/> <h4>Payment instruction can be viewed <a href=".$url.">here</a></h4>";  
    }
  ...
  public function redirectAction() {
    ...
    Veritrans_Config::$isProduction =
        Mage::getStoreConfig('payment/mandiribill/environment') == 'production'
        ? true : false;

    Veritrans_Config::$serverKey =
        Mage::getStoreConfig('payment/mandiribill/server_key_v2');
    ...
    if ($current_currency != 'IDR') {
      $conversion_func = function ($non_idr_price) {
          return $non_idr_price *
              Mage::getStoreConfig('payment/mandiribill/conversion_rate');
        };
    ...
    $payloads['payment_type']		 = 'echannel';
	  $payloads['echannel']		 = array(
											'bill_info1' => "Payment for",
                      'bill_info2' => "debt"
										);

    try {
      Mage::log($payloads,null,'vtdirect_veritrans.log',TRUE);
      $redirUrl = Veritrans_VtDirect::charge($payloads);
	  Mage::log(print_r($redirUrl,TRUE),null,'vtdirect_veritrans.log',TRUE);
      if($redirUrl->status_code=='201') {
        /* send an order email when redirecting to payment page although payment
       has not been completed. */
		$order->setState(Mage::getStoreConfig('payment/mandiribill/'),true,
			'New order, waiting for payment.');
		$this->send_new_order_mail(Mage::app()->getStore()->getStoreId(), $order, $order_billing_address, $order->getPayment(), $customer_details['email'] , $order_billing_address->getName(), $order->getCustomerIsGuest(), $redirUrl->permata_va_number, $redirUrl->gross_amount, $redirUrl->bill_key, $redirUrl->biller_code );
    ...
    Mage::getSingleton('core/session')->setBillkey($redirUrl->bill_key);
    Mage::getSingleton('core/session')->setBillercode($redirUrl->biller_code);
