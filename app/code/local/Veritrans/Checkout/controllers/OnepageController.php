...
/**
* IONS Custom Onepage controller for checkout
*
* @author IONS-TECH <developer@ions-tech.com>
*/
class Veritrans_Checkout_OnepageController extends Mage_Checkout_OnepageController {
    ...
    public function saveOrderAction()
    {
       ...
       /**
        ...
        if (isset($redirectUrl)) {
			$result['redirect'] = $redirectUrl;
			//unset order_reserved_id
			$quote = $this->getOnepage()->getQuote();
			$q_data = $this->getOnepage()->getQuote()->getData();
			$quote->setReservedOrderId(null);//reset the reserved order id
			//it seems that is_active was also set to 0, so in order to keep the basket we need is_active=1
			$quote->setIsActive(1);//keep the quote active
			$quote->save();
			} else {
				$this->getOnepage()->getQuote()->save();
